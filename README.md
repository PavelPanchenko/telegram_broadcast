...
**Примечание:** Токен бота больше не требуется в `.env` файле. Боты добавляются через веб-интерфейс после первого входа в систему.
...
Все данные сохраняются в SQLite базе данных `server/data/database.db`:
...
**Требования:**
- VPS сервер с установленными Docker и Docker Compose (плагин)
...
2. **Установите Docker и Docker Compose** (если еще не установлены):
   ...
   # Установка Docker Compose (плагин, входит в Docker Desktop и новые версии Docker)
   # Для старых систем может потребоваться установка отдельно
...
4. **Создайте файл `.env` в корне проекта:**
   ...
   **Примечание:** Токен бота больше не требуется в `.env`. Боты добавляются через веб-интерфейс после входа в систему.
...
5. Добавьте переменные окружения:
   ...
   - `SESSION_SECRET`
   - `NODE_ENV=production`
   - `COOKIE_SECURE=false` (или `true` для HTTPS)
...
1. **Войдите в систему** с учетными данными по умолчанию: `admin` / `admin`
2. **Добавьте ботов** через веб-интерфейс (кнопка "Добавить бота")
3. **Обновите CORS настройки** в `server/index.js` для вашего frontend домена
4. **Обновите `vercel.json`** с правильным URL backend
5. **Настройте сессии** - используйте внешнее хранилище (Redis) для продакшн
6. **Настройте файловое хранилище** - используйте S3 или подобное для `uploads/`
